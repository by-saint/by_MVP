/* ===================================================================
   ...
   Funcionalidades:
   - Correções de sintaxe que causavam "A carregar..." / layout branco.
   - Implementação do modal de Troca de Alimentos integrada.
=================================================================== */
        ...
        (w.days || []).forEach((d, di) => {
          (d.meals || []).forEach((m, mi) => {
            ...
            (m.components || []).forEach(c => {
              ...
              const finalNewGrams = Math.max(0, Math.round(newGrams / 5) * 5); // Arredonda para 5g, sem negativo
              const finalNewKcal = Math.round(((newFood?.nutrition?.kcal || 0) / 100) * finalNewGrams);
