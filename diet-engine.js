/* ===================================================================
    ARQUIVO: diet-engine.js (O "CÉREBRO") - v4.0 (TAF + Trocas)
    
    Resumo das Mudanças:
    - [Falha 4/5] Adicionados ~15 novos alimentos (vegetais, frutas, etc.)
      como EXEMPLO de como expandir o DB. O ideal é adicionar 200+.
    - [Falha 5] Corrigida a função `get` para usar um "top 5" em vez de
      "top 2", aumentando drasticamente a variedade (corrige "só alface").
    - [Falha 1] Corrigida a Coesão (ex: "Leite com Pipoca") movendo 'pipoca'
      para uma categoria própria e ajustando os 'BLUEPRINTS' do café.
    - [Falha 2] Corrigida a função `quantize` (dentro de computePortion...)
      para arredondar porções de "copo", "fatia", etc. (corrige "4.25 copos").
    - [Falha 3] Corrigida a lógica de contagem de refeições. Agora
      "Jantar" é sempre fixo e os lanches são adicionados com base nos dias.
    - [Melhoria 1] Adicionado reconhecimento da meta "TAF" em
      `analyzeMasterGoal` e `deriveTargets`.
    - [Melhoria 2] Adicionada a nova função `recalculateMealWithSwap`
      para o sistema de trocas.
    - [Melhoria 2] Adicionada a nova função `getFoodDatabase`
      para a UI de trocas.
=================================================================== */

const SuperDietEngine = (function(){
    /* ========== FOOD DB (v5.1 - EXEMPLO de Expansão) ========== */
    const v5_FoodDatabase = {
        ...
        // [Falha 4] Exemplo: Adicionando mais carnes
        'carne_acem_cozido': {
            name: 'Carne, Acém cozido (sem gordura)',
            nutrition: { kcal: 215, protein_g: 30, carb_g: 0, fat_g: 10, fiber_g: 0 },
            price_per_kg: 35.0, category: 'proteina_main', ig: 0,
            portion: { name: 'porção(ões)', grams: 100 }
        },
        ...
        // [Falha 4] Exemplo: Adicionando mais peixes
        'peixe_merluza_cozida': {
            name: 'Peixe, Merluza cozida',
            nutrition: { kcal: 98, protein_g: 21, carb_g: 0, fat_g: 1.2, fiber_g: 0 },
        ...
        // [Falha 4] Exemplo: Adicionando mais carbos
        'pure_batata': {
            name: 'Purê de batata (simples)',
            nutrition: { kcal: 88, protein_g: 2, carb_g: 18, fat_g: 0.8, fiber_g: 1.5 },
            price_per_kg: 5.0, category: 'cereal_main', ig: 85,
            portion: { name: 'colher(es) de servir', grams: 60 }
        },
        ...
        // [Falha 4] Exemplo: Adicionando mais tuberculos
        'mandioquinha_cozida': {
            name: 'Mandioquinha (Batata-baroa), cozida',
            nutrition: { kcal: 80, protein_g: 0.9, carb_g: 18.9, fat_g: 0.1, fiber_g: 1.8 },
            price_per_kg: 8.0, category: 'tuberculo', ig: 50,
            portion: { name: 'porção(ões)', grams: 100 }
        },
        ...
        'tapioca_goma': {
            ...
            price_per_kg: 10.0, category:'pao_snack', ig: 80, // Movido para 'pao_snack' para coesão
            portion: { name: 'porção(ões)', grams: 100 }
        },
        
        // [Falha 1] Pipoca movida para categoria própria para evitar combinações bizarras
        'milho_pipoca_estourada': {
            ...
            price_per_kg: 14.0, category:'cereal_snack_other', ig: 55, // <-- MUDANÇA DE CATEGORIA
        ...
        // [Falha 4] Exemplo: Adicionando mais frutas
        'abacaxi_cru': {
            name:'Abacaxi, cru',
        ...
        'kiwi_cru': {
            name: 'Kiwi, cru',
            nutrition: { kcal: 61, protein_g: 1.1, carb_g: 14.7, fat_g: 0.5, fiber_g: 3.0 },
            price_per_kg: 18.0, category: 'fruta', ig: 52,
            portion: { name: 'unidade(s)', grams: 80 }
        },
        'pera_crua': {
            name: 'Pera, crua com casca',
            nutrition: { kcal: 57, protein_g: 0.4, carb_g: 15.2, fat_g: 0.1, fiber_g: 3.1 },
            price_per_kg: 9.0, category: 'fruta', ig: 38,
            portion: { name: 'unidade(s)', grams: 150 }
        },
        ...
        // [Falha 4/5] Exemplo: Adicionando mais vegetais (essencial p/ micros)
        'abobrinha_refogada': {
        ...
        'pimentao_amarelo_cru': {
            name: 'Pimentão, Amarelo cru',
            nutrition: { kcal: 27, protein_g: 0.9, carb_g: 6.3, fat_g: 0.2, fiber_g: 1.2 },
            price_per_kg: 10.0, category: 'verdura', ig: 15,
            portion: { name: 'porção(ões)', grams: 50 }
        }
    }; // end v5_FoodDatabase
    ...
    // [Melhoria 2] Nova função para exportar o DB para a UI
    function getFoodDatabase() {
        // Retorna uma cópia para evitar mutações
        return JSON.parse(JSON.stringify(v5_FoodDatabase));
    }
    ...
    function extractRestrictions(profile){
        ...
        const restrictionMap = {
            ...
            'carne vermelha': ['carne_patinho_moido','carne_alcatra_grelhada','carne_coxao_mole_cozido', 'carne_acem_cozido'],
            'carne': ['carne_patinho_moido','carne_alcatra_grelhada','carne_coxao_mole_cozido','porco_lombo_grelhado', 'carne_acem_cozido'],
    ...
    // [Melhoria 1] Novas palavras-chave para TAF
    const KEYWORDS_TAF = ['taf', 'teste de aptidao fisica', 'exame fisico', 'barra', 'flexao', 'abdominal', 'corrida 12 min', 'teste fisico'];
    ...
    function analyzeMasterGoal(profile){
        ...
        const scoreTAF = scoreKeywords(text, KEYWORDS_TAF); // [Melhoria 1]
        ...
        if(scoreHealth > 0){
            ...
            // ... (lógica de saúde existente)
        
        // [Melhoria 1] Bloco TAF adicionado (ANTES de performance geral)
        } else if (scoreTAF > 0) {
            strategy.primaryGoal = 'PERFORMANCE';
            strategy.specificGoal = 'TAF_PREP'; // Nova meta específica
            strategy.proteinProfile = 'HIGH';
            strategy.workoutFocus = 'HYBRID_STRENGTH_ENDURANCE';
            
            // TAF é sobre performance com peso corporal.
            // Se o usuário quer perder peso (comum), é déficit. Se não, manutenção.
            const isFatLoss = text.includes('perder') || text.includes('emagrecer') || text.includes('secar');
            if (isFatLoss) {
            ...
            // ... (lógica híbrida existente)
        } else if (scorePerf > 0) {
            strategy.primaryGoal = 'PERFORMANCE';
            // ... (lógica de performance existente)
        }
        else {
    ...
    function deriveTargets(profile, strategy) {
        ...
        // [Melhoria 1] Garante que TAF_PREP nunca tenha superávit
        if (strategy && strategy.specificGoal === 'TAF_PREP' && targetCalories > tdee) {
            targetCalories = tdee; // Limita a manutenção
        }
    ...
    function _rankFoodsByCategory(categories, profile, shortTermAvoidList = [], longTermPenalizeList = [], restrictionList = []){
        ...
        // Filtra alimentos com rank 0 (restritos)
        return recs.filter(r => r.rank > 0).map(r => r.id);
    }
    ...
    function computePortionAmountsForMeal(foodDBFindFn, mealMacroTargets = {}, components = {}) {
        ...
        // [Falha 2] Função `quantize` movida para cá e melhorada
        const quantize = (v, portionName) => {
            const name = (portionName || '').toLowerCase();
            // Arredonda para 0.5 (meia porção)
            if (name.includes('copo') || name.includes('unidade') || name.includes('fatia') || name.includes('filé') || name.includes('scoop') || name.includes('pote') || name.includes('prato')) {
                return Math.max(0.5, Math.round(v * 2) / 2);
            }
            // Arredonda para 1 (sem "meia colher")
            if (name.includes('colher') || name.includes('concha')) {
                return Math.max(1, Math.round(v));
            }
            // Mantém 0.25 para porções genéricas (ex: 1.25 x 100g)
            return Math.max(0.25, Math.round(v * 4) / 4);
        };

        numProtPortions = quantize(numProtPortions || 0.5, protRec?.portion?.name);
        numCarbPortions = quantize(numCarbPortions || 0.5, carbRec?.portion?.name);
    ...
    /* ========== planLongTerm (v11.0) with blueprints & portion logic ========== */
    async function planLongTerm(profile, options = {}) {
        ...
        // BLUEPRINTS (v7.1) -- [Falha 1] Ajustado para coesão
        const BLUEPRINTS = {
            'café': [
                { name: 'Padrão', slots: { carb: 'pao_snack', prot: 'snack_prot', veg: 'fruta' } }, // Pão com ovo/queijo + fruta
                { name: 'Shake', slots: { prot: 'suplemento', carb: 'fruta', veg: 'gordura_boa' } }, // Whey com banana e pasta
                { name: 'Cereal', slots: { carb: 'cereal_snack', prot: 'laticinio_snack', veg: 'fruta' } }, // Aveia com Iogurte e fruta
                { name: 'Reforçado', slots: { prot: 'proteina_main', carb: 'tuberculo', veg: null } } // Ex: Frango com batata doce
            ],
            'almoço': [
            ...
            'lanche': [
                ...
                { name: 'Salgado', slots: { carb: 'pao_snack', prot: 'snack_prot', veg: null } } // Ex: Tapioca com queijo
            ],
            'jantar': [
        ...
        const SLOT_MAP = {
            ...
            'pao_snack': ['pao_snack'],
            'cereal_snack_other': ['cereal_snack_other'] // Categoria da pipoca
        };

        // Build master rotations once (fullRotation lists) - pass restrictions
        const cats = Object.keys(SLOT_MAP); // Usa todas as categorias do SLOT_MAP
        const fullRotation = {};
        ...
        // [Falha 5] CORREÇÃO da função `get` (Roleta Russa)
        // Agora usa um "pool" do top 5, mas ainda é determinístico
        const get = (list, idx) => {
            if (!list || list.length === 0) return null;
            const topN = 5; // Pega do Top 5
            const effectiveSize = Math.min(list.length, topN);
            if (effectiveSize === 0) return null;
            const pickIndex = Math.abs(idx) % effectiveSize; // 0, 1, 2, 3, 4, 0, 1, ...
        ...
        for(let w=0; w<totalWeeks; w++){
            ...
            for(let d=0; d<7; d++){
                ...
                // [Falha 3] Lógica de refeições CORRIGIDA
                const mealNames = ["Café da manhã", "Almoço", "Jantar"];
                if (diasTreinoCount >= 2) {
                    mealNames.splice(2, 0, "Lanche tarde"); // Adiciona Lanche Tarde
                }
                if (diasTreinoCount >= 4) {
                    mealNames.splice(1, 0, "Lanche manhã"); // Adiciona Lanche Manhã
                }
                if (diasTreinoCount >= 6) {
                ...
                // ... (Criação dos candidate pools, sem mudanças) ...
                ...
                for(let m=0; m<mealsCount; m++){
                    ...
                    // BLUEPRINT selection for this meal - CONTEXTUAL (v11.0)
                    let blueprintPool = [];
                    if (lname.includes('café')) {
                        ...
                        if (isAdvanced && goalIsMuscle) {
                            blueprintPool = BLUEPRINTS['café']; // todas 4 opções
                        } else {
                            blueprintPool = [BLUEPRINTS['café'][0], BLUEPRINTS['café'][1], BLUEPRINTS['café'][2]]; // Padrão, Shake, Cereal
                        }
                        ...
                        blueprintPool = [BLUEPRINTS['lanche'][0], BLUEPRINTS['lanche'][1], BLUEPRINTS['lanche'][3]]; // Padrão, Proteico, Salgado
                         if(userHasSupplement(profile, 'whey')) {
                            blueprintPool.push(BLUEPRINTS['lanche'][2]); // Adiciona "Rápido" (whey)
                         }
                    }
                    ...
                    // Helper para buscar candidatos
                    const getCandidates = (slotType, recentPicks, longTermCats) => {
                        if (!slotType) return [];
                        const cats = SLOT_MAP[slotType] || [];
                        const longTerm = [].concat(...cats.map(cat => longTermPenalize[cat] || []));
                        return _rankFoodsByCategory(cats, profileWithTargets, recentPicks, longTerm, restrictions);
                    };

                    const protCandidates = getCandidates(protSlotType, recentProteinPicks, [protSlotType]);
                    const carbCandidates = getCandidates(carbSlotType, recentCarbPicks, [carbSlotType]);
                    const vegCandidates  = getCandidates(vegSlotType, [], [vegSlotType]);

                    let protPick = protCandidates.length > 0 ? get(protCandidates, pickIdx) : null;
                    let carbPick = carbCandidates.length > 0 ? get(carbCandidates, pickIdx + 1) : null;
                    let vegPick  = vegCandidates.length > 0 ? get(vegCandidates, pickIdx + 2) : null;

                    // Fallbacks (se o blueprint falhar)
                    if (lname.includes('almoço') || lname.includes('jantar')) {
                        if (!protPick) protPick = getFirst(fullRotation['proteina_main']) || fallbackProtein;
                        if (!carbPick) carbPick = getFirst(fullRotation['cereal_main']) || fallbackCarb;
                        if (!vegPick) vegPick = getFirst(fullRotation['leguminosa']) || getFirst(fullRotation['verdura']);
                    }
                    if (lname.includes('café') || lname.includes('lanche')) {
                         if (!protPick) protPick = getFirst(fullRotation['snack_prot']) || fallbackSnackProt;
                         if (!carbPick) carbPick = getFirst(fullRotation['snack_carb']) || fallbackSnackCarb;
                    }
                    
                    // [Falha 1] O passo de coesão foi movido para os BLUEPRINTS,
                    // que agora são mais específicos (ex: 'Cereal' casa com 'laticinio_snack')
                    // Não é mais necessário um passo de coesão complexo aqui.
                    ...
                    if(isTraining && wantsWhey && (/jantar|ceia/.test(lname)) && selectedBlueprint.name !== 'Shake'){
                         // Adiciona whey pós-treino (Jantar/Ceia)
                         extraComponents.push({ food: (v5_FoodDatabase['whey_concentrado'] ? 'whey_concentrado' : 'whey_protein_concentrado'), grams: 30, role:'suplemento' });
                    }
                    ...
                    meals.push({
                        ...
                        // [Melhoria 2] Adiciona um ID único para cada refeição
                        meal_uuid: `w${w}d${d}m${m}` 
                    });
                } // end meals loop

                recentProteinPicks = [...new Set([...recentProteinPicks, ...dayProteinPicks])]; // usa Set para evitar duplicados
                recentCarbPicks = [...new Set([...recentCarbPicks, ...dayCarbPicks])];
        ...
        const payload = {
            version: 'super-algo-v11.0-TAF-fixes',
            created_at: nowISO(),
            ...
            provenance: { food_db_version: 'v5.1_FoodDatabase_expanded', built_with: 'super-diet-engine-v11.0' }
        };
    ...
    /* Função 'deleteLatestPlan' (sem mudanças) */
    ...
    /* ==========================================================
        [Melhoria 2] NOVA FUNÇÃO para Troca de Alimentos
       ========================================================== */
    function recalculateMealWithSwap(meal, oldFoodSourceId, newFoodSourceId) {
        try {
            const newMeal = JSON.parse(JSON.stringify(meal)); // Cópia profunda
            const newFoodRec = findFood(newFoodSourceId);
            
            if (!newFoodRec) {
                console.error("Novo alimento não encontrado:", newFoodSourceId);
                return meal; // Retorna original se o novo alimento não existe
            }
            ...
            // Encontra o alimento antigo e remove, guardando suas calorias e "role"
            newMeal.components = newMeal.components.filter(c => {
                if (c.source_id === oldFoodSourceId) {
                    targetKcal = c.kcal || 0;
                    oldRole = c.role || 'extra';
                    found = true;
                    return false; // Remove
                }
                return true; // Mantém
            ...
            if (!found) {
                console.error("Alimento antigo não encontrado no 'meal':", oldFoodSourceId, meal);
                return meal; // Alimento antigo não encontrado
            }
            
            // Calcula quantos gramas do NOVO alimento são necessários para bater as calorias
            const newFoodKcalPer100g = _safeNum(newFoodRec.nutrition?.kcal, 0);
            if (newFoodKcalPer100g <= 0) {
                 console.error("Novo alimento tem 0 kcal:", newFoodSourceId);
                 return meal; // Evita divisão por zero
            }
            
            const newGrams = Math.round((targetKcal / newFoodKcalPer100g) * 100);
            
            // Adiciona o novo alimento
            newMeal.components.push({
                food: stripParenthesis(newFoodRec.name),
                grams: newGrams,
                role: oldRole, // Mantém o "role" (prot, carbo, etc)
                source_id: newFoodSourceId,
                kcal: targetKcal // Atribui as calorias originais para manter o total
            });
            
            // Recalcula o total de calorias da refeição (só por segurança, deve ser o mesmo)
            newMeal.mealKcalTotal = newMeal.components.reduce((sum, c) => sum + (c.kcal || 0), 0);
            
            // Recalcula as porções (se o novo alimento tiver porção)
            const portionGrams = _safeNum(newFoodRec.portion?.grams, 100);
            const numPortions = (portionGrams > 0) ? (newGrams / portionGrams) : 0;
            
            const portionName = newFoodRec.portion?.name || 'porção';
            
            // [Falha 2] Copiando a função `quantize` melhorada
            const quantize = (v, pName) => {
            ...
            const finalPortions = quantize(numPortions, portionName);

            // Atualiza o gramsComputed (para a UI)
            // Garante que gramsComputed exista
            if (!newMeal.gramsComputed) {
                newMeal.gramsComputed = { details: {} };
            }
            if (!newMeal.gramsComputed.details) {
                 newMeal.gramsComputed.details = {};
            }

            if (oldRole === 'proteina') {
                newMeal.gramsComputed.details.prot_portions = finalPortions;
                newMeal.gramsComputed.details.prot_portion_name = portionName;
            } else if (oldRole === 'carbo') {
                newMeal.gramsComputed.details.carb_portions = finalPortions;
                newMeal.gramsComputed.details.carb_portion_name = portionName;
            } else if (oldRole === 'legume') {
                 // Lógica para 'veg' se necessário (estava faltando)
                 newMeal.gramsComputed.details.veg_portions = finalPortions;
                 newMeal.gramsComputed.details.veg_portion_name = portionName;
            }
            
            return newMeal;
        } catch(err) {
            console.error("Erro em recalculateMealWithSwap:", err);
            return meal; // Retorna a refeição original em caso de erro
        }
    }
    ...
    return {
        ...
        deriveTargets,
        detectGoalType,
        // [Melhoria 2] Exportando as novas funções
        getFoodDatabase,
        recalculateMealWithSwap
    };
